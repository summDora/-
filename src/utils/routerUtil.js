    ...
    if (!router) {
      router =
        typeof item === "string"
          ? { path: item, name: item }
          : {
              path: item.pcode == 0 ? item.id + "_key" : "/" + item.url,
              name: item.name,
              component:
                item.pcode == 0
                  ? (resolve) => {
                      require([`@/layouts/tabs/TabsView`], resolve);
                    }
                  : (resolve) => {
                      require([`@/pages/${item.url}`], resolve);
                    },
            };
    }
...
function loadRoutes(routesConfig) {
  ...
  let menuRoutes = router.children;
  if (menuRoutes) {
