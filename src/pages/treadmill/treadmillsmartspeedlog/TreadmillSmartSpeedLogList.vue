...
<script>
  ...
  import TableForm from './TreadmillSmartSpeedLogForm'
  const Columns = [
              {
          title: "",
          dataIndex: "id",
  ...
  export default {
    ...
    methods: {
      ...
      getTableData() {
        ...
        postForm("/treadmill/treadmillsmartspeedlog/page", params).then((res) => {
      ...
      // 删除菜单
      deleteRecord(id) {
        post("/treadmill/treadmillsmartspeedlog/delete", [id]).then((res) => {
          if (res.code == 200) {
            this.getTableData();
            this.$message
                    .success(res.msg);
