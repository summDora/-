...
<script>
  ...
  import TableForm from './TreadmillWhiteForm'
  const Columns = [
              {
          title: "id",
          dataIndex: "id",
  ...
  export default {
    ...
    methods: {
      ...
      getTableData() {
        ...
        postForm("/treadmill/treadmillwhite/page", params).then((res) => {
      ...
      // 删除菜单
      deleteRecord(id) {
        post("/treadmill/treadmillwhite/delete", [id]).then((res) => {
          if (res.code == 200) {
            this.getTableData();
            this.$message
                    .success(res.msg);
