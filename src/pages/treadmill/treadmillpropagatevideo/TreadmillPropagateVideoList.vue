...
<script>
  ...
  import TableForm from './TreadmillPropagateVideoForm'
  const Columns = [
              {
          title: "ID",
          dataIndex: "id",
  ...
  export default {
    ...
    methods: {
      ...
      getTableData() {
        ...
        postForm("/treadmill/treadmillpropagatevideo/page", params).then((res) => {
      ...
      // 删除菜单
      deleteRecord(id) {
        post("/treadmill/treadmillpropagatevideo/delete", [id]).then((res) => {
          if (res.code == 200) {
            this.getTableData();
            this.$message
                    .success(res.msg);
