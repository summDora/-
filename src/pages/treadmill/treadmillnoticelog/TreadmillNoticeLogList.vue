...
<script>
  ...
  import TableForm from './TreadmillNoticeLogForm'
  const Columns = [
              {
          title: "ID",
          dataIndex: "id",
  ...
  export default {
    ...
    methods: {
      ...
      getTableData() {
        ...
        postForm("/treadmill/treadmillnoticelog/page", params).then((res) => {
      ...
      // 删除菜单
      deleteRecord(id) {
        post("/treadmill/treadmillnoticelog/delete", [id]).then((res) => {
          if (res.code == 200) {
            this.getTableData();
            this.$message
                    .success(res.msg);
