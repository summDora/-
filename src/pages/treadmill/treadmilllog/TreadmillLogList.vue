...
<script>
  ...
  import TableForm from './TreadmillLogForm'
  const Columns = [
              {
          title: "",
          dataIndex: "id",
          ...
          key: "sn",
              ...
              {
          title: "类型,1跑步机状态,2,开机,3,用户下发关机,4,跑步机上传关机,5,续费",
          dataIndex: "type",
          key: "type",
        },
              {
  ...
  export default {
    ...
    methods: {
      ...
      getTableData() {
        ...
        postForm("/treadmill/treadmilllog/page", params).then((res) => {
      ...
      // 删除菜单
      deleteRecord(id) {
        post("/treadmill/treadmilllog/delete", [id]).then((res) => {
          if (res.code == 200) {
            this.getTableData();
            this.$message
                    .success(res.msg);
