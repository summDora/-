...
<script>
  ...
  import TableForm from './AppUserPayBindForm'
  const Columns = [
              {
          title: "id",
          dataIndex: "id",
  ...
  export default {
    ...
    methods: {
      ...
      getTableData() {
        ...
        post("/app/appuserpaybind/page", params).then((res) => {
      ...
      // 删除菜单
      deleteRecord(id) {
        post("/app/appuserpaybind/delete", [id]).then((res) => {
          if (res.code == 200) {
            this.getTableData();
            this.$message
                    .success(res.msg);
      ...
      select(){
        ...
        get('/app/appuserpaybind/select',this.CdtionSearch).then(res=>{
          console.log(res)
        })
      }
