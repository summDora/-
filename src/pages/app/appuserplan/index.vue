...
<script>
  ...
  import TableForm from './AppUserPlanForm'
  const Columns = [
              {
          title: "id",
          dataIndex: "id",
  ...
  export default {
    ...
    methods: {
      ...
      getTableData() {
        ...
        post("/app/appuserplan/page", params).then((res) => {
      ...
      // 删除菜单
      deleteRecord(id) {
        post("/app/appuserplan/delete", [id]).then((res) => {
          if (res.code == 200) {
            this.getTableData();
            this.$message
                    .success(res.msg);
      ...
      select(){
        ...
        get('/app/appuserplan/select',this.CdtionSearch).then(res=>{
          console.log(res)
        })
      }
