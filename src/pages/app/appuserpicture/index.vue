...
<script>
  ...
  import TableForm from './AppUserPictureForm'
  const Columns = [
              {
          title: "id",
          dataIndex: "id",
  ...
  export default {
    ...
    methods: {
      ...
      getTableData() {
        ...
        post("/app/appuserpicture/page", params).then((res) => {
      ...
      // 删除菜单
      deleteRecord(id) {
        post("/app/appuserpicture/delete", [id]).then((res) => {
          if (res.code == 200) {
            this.getTableData();
            this.$message
                    .success(res.msg);
      ...
      select(){
        ...
        get('/app/appuserpicture/select',this.CdtionSearch).then(res=>{
          console.log(res)
        })
      }
