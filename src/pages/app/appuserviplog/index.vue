...
<script>
  ...
  import TableForm from './AppUserViplogForm'
  const Columns = [
              {
          title: "id",
          dataIndex: "id",
          ...
          title: "treadmill_id",
          dataIndex: "treadmillId",
          key: "treadmillId",
        },              {
          title: "vip_start_date",
  ...
  export default {
    ...
    methods: {
      ...
      getTableData() {
        ...
        post("/app/appuserviplog/page", params).then((res) => {
      ...
      // 删除菜单
      deleteRecord(id) {
        post("/app/appuserviplog/delete", [id]).then((res) => {
          if (res.code == 200) {
            this.getTableData();
            this.$message
                    .success(res.msg);
      ...
      select(){
        ...
        get('/app/appuserviplog/select',this.CdtionSearch).then(res=>{
          console.log(res)
        })
      }
