...
<script>
  ...
  import TableForm from './AppUserFansForm'
  const Columns = [
              {
          title: "Id",
          dataIndex: "id",
  ...
  export default {
    ...
    methods: {
      ...
      getTableData() {
        ...
        postForm("/app/appuserfans/page", params).then((res) => {
      ...
      // 删除菜单
      deleteRecord(id) {
        post("/app/appuserfans/delete", [id]).then((res) => {
          if (res.code == 200) {
            this.getTableData();
            this.$message
                    .success(res.msg);
